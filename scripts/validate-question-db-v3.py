#!/usr/bin/env python3
"""Validate data/question-db-v3.json against Phase 1 Rebuild v3 rules."""
...
DATA = ROOT / "data"
QUESTION_DB = DATA / "question-db-v3.json"
PATTERN_DB = DATA / "pattern-db-v3.json"
STATISTICS_DB = DATA / "statistics-v3.json"
...
def main() -> int:
    ...
    if not QUESTION_DB.exists():
        print("FAIL: question-db-v3.json 없음")
        return 1
    ...
    if len(patterns) == 0:
        fail("pattern-db-v3.json 비어 있음", failures)
    if len(statistics) == 0:
        fail("statistics-v3.json 비어 있음", failures)

    expected_total = len(YEARS) * EXPECTED_PER_YEAR
    ...
    print("Question DB v3 Validation")
